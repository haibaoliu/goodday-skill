...
### GBrain (garrytan/gbrain) — Thin Harness + Fat Skills + Fat Data
Garry Tan's personal knowledge brain (⭐18,244, MIT). Not a memory plugin — a complete agent knowledge architecture. Core innovation: Markdown skills as method calls, brain pages with mandatory back-links and source citations, read-only subagent trust model for untrusted content processing (Book Mirror). Strong alignment with Hermes' skill system — same SKILL.md format, similar subagent/cron primitives.

See `references/gbrain-architecture.md` for full architecture breakdown, Book Mirror pipeline, brain page data model, and Hermes comparison.
...
- `references/gbrain-architecture.md` — GBrain architecture: Thin Harness + Fat Skills + Fat Data, Book Mirror, brain page model, Hermes comparison (2026-05-23)
- `references/agentmemory-analysis.md` — agentmemory (rohitg00, ⭐21.8k): 记忆服务器深度分析 — 4层记忆巩固、BM25+向量+图 RRF融合、95.2% R@5、Hermes 原生插件、12 hooks 自动捕获 (2026-06-08)
