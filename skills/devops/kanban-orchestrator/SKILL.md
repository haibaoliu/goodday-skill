...
**Dead-end check (before finalizing any task direction):** Read `<project>/.hermes/dead-ends.md` if it exists. For each candidate task approach:

- If the approach matches a `severity: block` entry → **discard** that direction, note the recorded alternative.
- If it matches a `severity: warn` entry → include in the task body: `⚠️ Dead-end warning: <approach> previously failed because <reason>. Only proceed if your approach differs in <specific way>.`
- If no registry file exists → proceed normally (first run).
...
## Stagnation detection & auto-replan

When the orchestrator is re-invoked (user says "check progress" or a lane appears stuck), run the stagnation check before adding new tasks.

### What counts as "no-improvement"

A task in a lane produced zero effective progress if:

| Outcome | Condition |
|---|---|
| `completed` | Metadata shows no measurable change (e.g., `benchmark_delta: 0`, `tests_passed` unchanged from parent, summary says "no improvement found") |
| `cancelled` | Approach-level failure (not OOM/timeout/credential — those are infra, not stagnation) |
| `completed` | Worker completed but summary is "explored X, found nothing" |

Do NOT count as no-improvement: `blocked` (waiting for human), `timed_out` (infra), `crashed` (infra).

### Threshold

**3 consecutive no-improvement tasks in the same lane → lane is stagnated.**

### Replan actions (in order of preference)

When a lane stagnates:

1. **Read the dead-end registry** for that lane. Extract the recorded alternatives and reasons.
2. **Propose one of:**
   - **Merge**: Combine this lane's remaining work into a sibling lane. The sibling lane gets the stagnated lane's accumulated knowledge.
   - **Split**: The current direction is too coarse. Decompose into 2-3 finer sub-directions, each a new task.
   - **Redirect**: Take the recorded alternative from the dead-end registry and create a new task for that direction.
   - **Abandon**: If all recorded alternatives are exhausted, mark the lane as a dead-end and report to the user.
3. **Create the replan tasks** with explicit references to what didn't work:
   ```
   body="⚠️ This lane replaces [stagnated-lane]. Previous 3 attempts (t_X, t_Y, t_Z) produced no improvement. 
   Dead-end registry shows: [reasons]. Try [alternative direction]."
   ```
4. **Report to user**: "Lane `<name>` stagnated after 3 no-improvement tasks. Replanned: [action taken]."

### When to run the check

- User asks for status update
- You're about to add new tasks and notice many completed/cancelled tasks in a lane
- A synthesis/integration task reports "nothing to integrate"

### Integration with dead-end registry

Stagnation detection and dead-end registry are complementary:
- **Dead-end registry** prevents *repeating specific approaches*
- **Stagnation detection** prevents *spinning wheels on a direction*

When a lane stagnates, its last 3 tasks should already have dead-end entries (workers record before cancelling). If they don't, the stagnation report is your signal to create them retroactively from task summaries.
